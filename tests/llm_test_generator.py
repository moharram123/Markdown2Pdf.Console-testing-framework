...
# ---------------------------------------------------------------------------
# Pytest integration
# The functions below wrap the standalone script logic so that the LLM
# test-generation step appears in the pytest HTML report alongside all other
# test results.  The workflow step "Run LLM test generator" in quality-gate.yml
# is therefore no longer needed as a separate script call.
# ---------------------------------------------------------------------------
import pytest


def test_llm_test_generation_runs():
    """Verify that the LLM test-generation pipeline completes without errors
    and produces the expected summary JSON file."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    try:
        results = run_llm_test_generation()
    except Exception as exc:
        pytest.skip(f"LLM test generation skipped (API unavailable): {exc}")
    assert isinstance(results, list), "run_llm_test_generation() must return a list"
    assert len(results) > 0, "Expected at least one generated test case"
    summary_path = OUTPUT_DIR / "llm_generated_tests_summary.json"
    assert summary_path.exists(), f"Summary file not found at {summary_path}"


def test_llm_vs_manual_comparison():
    """Verify that the LLM vs manual comparison runs and achieves at least
    50% average coverage (sanity threshold)."""
    summary_path = OUTPUT_DIR / "llm_generated_tests_summary.json"
    if not summary_path.exists():
        pytest.skip("LLM summary not available; run test_llm_test_generation_runs first")
    try:
        report = compare_llm_vs_manual()
    except Exception as exc:
        pytest.skip(f"LLM comparison skipped: {exc}")
    avg_coverage = report["summary"]["average_coverage"]
    assert avg_coverage >= 0.5, (
        f"LLM coverage too low: {int(avg_coverage * 100)}% "
        f"(expected >= 50%). Check llm_generated_tests_summary.json."
    )
